      ...
      <div className="flex gap-2 border-b border-border">
        {(['proposals', 'card'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-3 py-2 capitalize ${tab === t ? 'border-b-2 border-accent font-semibold' : 'text-muted'}`}>
      ...
      <section className="space-y-3">
        ...
        {accepted.length === 0 ? (
          ...
          <div className="flex flex-wrap items-end gap-2 text-sm">
            <select className="rounded border border-border px-2 py-1" value={draft.teamId}
            ...
            <input className="flex-1 rounded border border-border px-2 py-1" placeholder="Milestone title"
              value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} />
            <input type="number" min={1} className="w-20 rounded border border-border px-2 py-1"
        ...
        <ul className="space-y-2 text-sm">
          {milestones.filter((m) => m.taskId === taskId).map((m) => (
            <li key={m.id} className="flex items-center gap-3 rounded border border-border p-2">
              ...
              {m.confirmedByBusiness ? (
                ...
                <button className="rounded bg-accent px-3 py-1 text-accent-foreground" onClick={() => confirmMilestone(m.id)}>
                  Confirm milestone
                </button>
              )}
            </li>
