...
import { Quests, TeamProgress } from './TeamProgress';
...
export function StudentHome() {
  ...
  const { teams, currentTeamId, teamPoints, cards, proposals, milestones } = useStore();
  ...
  return (
    <div className="space-y-6">
      ...
      <TeamProgress points={teamPoints[team.id] ?? 0} />

      <section className="space-y-2">
        <h2 className="text-xl font-extrabold">Quests</h2>
        <Quests milestones={milestones.filter((m) => m.teamId === team.id)} cards={cards} />
      </section>

      <section className="space-y-3">
        <h2 className="text-xl font-extrabold">Projects for you</h2>
