...
let pc, dc, mic; const audio = new Audio(); audio.autoplay = true;
async function start() {
  ...
  dc.onopen = () => { document.getElementById('s').textContent = 'connected — speak'; dc.send(JSON.stringify({type:'response.create'})); };
  dc.onmessage = e => { const ev = JSON.parse(e.data);
    ...
    if (ev.type === 'response.function_call_arguments.done') { const a = JSON.parse(ev.arguments || '{}');
      ...
      if (ev.name === 'finish_interview') { log('finished'); setTimeout(stop, 4000); } else dc.send(JSON.stringify({type:'response.create'})); } };
